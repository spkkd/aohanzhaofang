<template>
	<view>
				...
				<view class="wrap">
					<u-form ref="uForm">
						...
						<u-form-item label-width="150" :label-position="labelPosition" label="朝向" >
							<u-input :border="border" type="select" :select-open="directionShow" v-model="model.directionVo" placeholder="请选择朝向" @click="directionShow = true"></u-input>
						</u-form-item>
						</u-form-item>
						...
						<u-form-item label-width="180" :label-position="labelPosition" label="付款方式"  required>
							<u-input :border="border" type="select" :select-open="payTypeSheetShow" v-model="model.payTypeVo" placeholder="请选择付款方式(必选)"
							 @click="payTypeSheetShow = true"></u-input>
						</u-form-item>
		...
		<u-action-sheet :list="payTypeSheetList" v-model="payTypeSheetShow" @click="payTypeSheetCallback"></u-action-sheet>
...
export default {
	...
	methods: {
		...
		// 点击actionSheet回调
		payTypeSheetCallback(index) {
			//uni.hideKeyboard();
			this.model.payTypeVo = this.payTypeSheetList[index].text;
		},
		// 选择类型回调
		selectConfirm(e) {
